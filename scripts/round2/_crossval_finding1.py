"""Cross-validate Finding 1 (OBI-extreme ACF veto) per-day.

If the +1,080 backtest gain is concentrated on one day → overfit.
If spread ~evenly across 3 days → real.
"""
...
def make_trader(obi_veto_thr=None):
...
# Per-day comparison: baseline vs veto at several thresholds
print(f"{'day':>4}  {'baseline_osm':>13}  {'veto0.60_osm':>13} {'Δ':>7}  {'veto0.65_osm':>13} {'Δ':>7}  {'veto0.70_osm':>13} {'Δ':>7}")
for day in [-1, 0, 1]:
    t_base = make_trader()
    _, osm_base = run_day(t_base, day)
    row = [f"{day:>+4d}", f"{osm_base:>13,.0f}"]
    for thr in [0.60, 0.65, 0.70]:
        t = make_trader(obi_veto_thr=thr)
        _, osm_v = run_day(t, day)
        delta = osm_v - osm_base
        row.extend([f"{osm_v:>13,.0f}", f"{delta:>+7,.0f}"])
    print("  ".join(row))

print("\nInterpretation: if delta is positive on all 3 days, the signal is robust.")
print("If it's concentrated on one day (e.g. +1000 on one, -100 on others), it's overfit.")
